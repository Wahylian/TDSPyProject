...
The streamer filters the manifest to one split, decodes each image to BGR, and
yields (image, label) pairs in a randomized order. These fabricate real decodable
files and a manifest under tmp_path, covering split validation, manifest loading,
the load path, the generator contract, BGR convention, feature integrity, and the
reproducible-but-global-RNG-safe shuffle.
"""
...
def _write_png(path, color=(40, 120, 200), width: int = 12, height: int = 8) -> str:
    """Write a solid-colour BGR PNG to path and return its path.

    color is in OpenCV's BGR order, exactly what cv2.imread returns on read-back.
    """
...
def _write_manifest(path, rows, header=("photo_name", "photo_path", "label", "split")):
    """Write a small manifest CSV; each row is a tuple matching header."""
    lines = [",".join(header)]
...
def _write_labeled_manifest(path, image_path, n):
    ...
    Every row points at the same image, so the label is the only identity tag for
    tracking where each datapoint lands after the shuffle.
    """
...
class TestSplitValidationAndConstants:
    ...
    def test_rejects_invalid_splits(self, bad):
        """Unknown, mis-cased, or empty split names are rejected."""
...
class TestManifestLoading:
    ...
    def test_filters_to_requested_split_with_int_labels(self, tmp_path):
        """Only the requested split's rows return, with paths and int labels aligned."""
...
class TestErrorHandling:
    ...
    def test_invalid_split_raises_before_reading(self, tmp_path):
        """An unknown split is rejected eagerly by get_feature_stream."""
        with pytest.raises(ValueError):
...
class TestGeneratorContract:
    ...
    def streamed_pairs(self, tmp_path):
        """Drain the stream for a 3-file, alternately-labelled train split."""
        rows = []
        for i in range(3):
    ...
    def test_yields_decoded_bgr_images(self, streamed_pairs):
        """The stream yields one (BGR image, int label) pair per file.

        Order is randomized, so labels are compared as an order-independent multiset.
        """
...
class TestRandomizedOrdering:
    """get_feature_stream randomizes order, reproducibly when seeded.

    Identity is tracked via the unique per-row label from _write_labeled_manifest
    (every row points at the same image, so the label is the only tag).
    """
    ...
    def test_seeded_order_matches_explicit_shuffle(self, tmp_path, shared_image):
        """A seeded stream matches an independent random.Random(seed) shuffle."""
        csv_path = str(tmp_path / "dataset_split.csv")
        _write_labeled_manifest(tmp_path / "dataset_split.csv", shared_image, 25)
    ...
    def test_does_not_perturb_global_rng(self, tmp_path, shared_image):
        ...
        A local random.Random instance is used, so a caller's global RNG sequence
        is unaffected.
        """
        csv_path = str(tmp_path / "dataset_split.csv")
