...
Pins that each moved symbol lives in its intended submodule, the package
re-exports the same objects (identity, not copies), the operation registry is
shared, and each split-out module still behaves end to end.
"""
...
class TestSymbolHoming:
    ...
    def test_pipeline_still_exposes_batch_level_ops_alias(self):
        """preprocessing.pipeline.BATCH_LEVEL_OPS keeps resolving."""
...
class TestModuleBehaviourUnchanged:
    ...
    def test_imagepipeline_fit_transform_matches_batch_process(self, image_batch):
        """A no-batch-op pipeline agrees via fit_transform and batch_process."""
        pipe = ip.ImagePipeline([
            ("grayscale", {}),
            ("resize", {"target_size": (16, 16)}),
            ("vectorize", {}),
