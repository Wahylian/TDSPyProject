...
collect_diagnostics gathers tree feature importances and OOB (tree models only),
the hyperparameter-grid scores (always), and an opt-in learning curve. These fit
real tiny searches and pin which keys populate for a tree vs a linear model, plus
...
class TestCollectDiagnostics:
    ...
    def test_random_forest_exposes_importances_and_oob(self, feature_split):
        """A tree model reports one importance per feature and an OOB score."""
        s = feature_split
        ...
        assert diag["hyperparameter_scores"]   # always populated
        json.dumps(diag)

    def test_linear_model_has_null_tree_diagnostics(self, feature_split):
        """A non-tree model leaves the tree-only diagnostics None (uniform schema)."""
        s = feature_split
    ...
    def test_include_curves_adds_learning_curve(self, feature_split):
        """With include_curves the learning curve is a well-formed dict, or None."""
        s = feature_split
        search = _fit_search("logreg", s)
        ...
        # Best-effort: may be None on this tiny split; if present it must be aligned.
        if lc is not None:
