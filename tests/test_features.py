...
Each unit is tested with heavy collaborators mocked, so nothing decodes a real
image or reads a manifest: build_feature_pipeline/_parse_pipeline_spec (pure
resolution), load_images (draining a mocked stream with the sample cap and
empty-stream guard), and fit_features/transform_features (fit-on-train /
transform-on-holdout plus the npz+joblib cache exercised via tmp_path). The
ImagePipeline is a light mock returning canned matrices.
"""
...
# A minimal valid custom spec (real op names so ImagePipeline accepts it).
...
class TestBuildFeaturePipeline:
    ...
    def test_spec_takes_precedence_over_name(self):
        """When both are given, the spec wins and the name is ignored."""
    ...
    def test_spec_with_unknown_op_raises_value_error(self):
        """A spec naming an op ImagePipeline rejects is a ValueError."""
...
class TestFitFeatures:
    ...
    def test_cache_hit_loads_without_reloading_images(self, tmp_path):
        ...
        # A trivial picklable object stands in for the fitted pipeline.
        joblib.dump(ImagePipeline([("grayscale", {})]), pipe_path)

        # This pipeline would explode if wrongly used to recompute.
...
class TestTransformFeatures:
    ...
    def test_no_cache_transforms_split(self, image_label_pairs):
        """With cache_dir=None the fitted pipeline's transform is applied per batch."""
...
class TestTrainCachePaths:
    ...
    def test_paths_encode_prefix_and_sample_cap(self, tmp_path):
        """The feature/pipeline paths embed the prefix + sample cap and suffixes."""
        ...
        # The directory is created as a side effect so later writes can't fail.
